'use strict';
// Bounded diagnostic: one new image, read-only access to the saved book.
...
async function main() {
  ...
  const results = [];
  for (const spread of [1]) {
    const text = story.spreads.find(s => s.spread === spread).text;
    const shot = shotPlan[spread];
    const template = await createTypographyTemplate({ childAge: profile.age, ink, text, side: shot.textSide, typographyScale: 1.5 });
    ...
    const prompt = buildCharacterPrompt(scene, 'pixar_premium', profile.name, text, stored.outfitSpec.text, manifest.context.characterDescription, null, null, {
      ...
      bookTextInk: ink, typographyTemplate: true, typographyScale: 1.5, typographyRef: refs.typographyRef,
