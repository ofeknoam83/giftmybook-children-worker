...
async function generateStoryText(childDetails, theme, customDetails, opts = {}) {
  ...
  if (theme === 'mothers_day') {
    if (additionalCoverCharacters) {
      ...
      systemPrompt += `\n\n⚠️ LOVE TO MOM OVERRIDE — TEXT-ONLY + SIGNATURE OBJECTS (MOM NEVER VISUALLY DRAWN): Mom is a co-protagonist of this story IN THE WORDS, but she has NO reference image and is NOT on the approved cover. Mom must NEVER appear visually in any spread illustration — not as a face, not as a body, not as a hand / arm / finger / shoulder / back-of-head / cropped torso / silhouette / shadow / reflection. Mom lives in (a) the manuscript text and (b) signature OBJECTS in the scene that imply her: a still-warm tea mug, a folded cardigan over a chair-back, an empty rocking chair, reading glasses on a side table, a worn paperback face-down, a pair of slippers.\n\nIn EVERY spread_image_prompt:\n- Show the HERO experiencing the moment alone in the scene, with one or two of Mom's signature objects placed naturally.\n- Examples: "The child sits on the kitchen floor with a coloring book; Mom's still-warm tea mug is on the table behind them." / "The child stands at the window watching rain; Mom's cardigan is folded over the armchair." / "The child closes a picture book on their lap; an empty rocking chair beside them holds Mom's reading glasses."\n- NEVER write "Mom hugs the child", "Mom's hands wrap around the child", "Mom kneels beside the child", "We see Mom from behind", "Mom's arm reaches into frame" — these all draw Mom and break the policy.\n- The TEXT can say warm things about Mom; the IMAGE shows the child's world with Mom's signature objects.\nOther family members — text only, never illustrated.`;
    }
    ...
    if (additionalCoverCharacters) {
      ...
      systemPrompt += `\n\n⚠️ FATHER'S DAY OVERRIDE — TEXT-ONLY + SIGNATURE OBJECTS (DAD NEVER VISUALLY DRAWN): Dad is a co-protagonist of this story IN THE WORDS, but he has NO reference image and is NOT on the approved cover. Dad must NEVER appear visually in any spread illustration — not as a face, not as a body, not as a hand / arm / finger / shoulder / back-of-head / cropped torso / silhouette / shadow / reflection. Dad lives in (a) the manuscript text and (b) signature OBJECTS in the scene that imply him: a folded jacket over a chair-back, a coffee mug, a casual wristwatch on the counter, a pair of work boots by the door, a worn baseball cap on a hook.\n\nIn EVERY spread_image_prompt:\n- Show the HERO experiencing the moment alone in the scene, with one or two of Dad's signature objects placed naturally.\n- Examples: "The child sits at the kitchen table with a small toy; Dad's coffee mug is beside the napkin holder." / "The child stands in the entryway lacing up small shoes; Dad's work boots wait on the mat." / "The child looks out the back door at the yard; Dad's baseball cap rests on a hook by the door."\n- NEVER write "Dad lifts the child", "Dad's hands rest on the child's shoulder", "We see Dad from behind", "Dad's arm reaches into frame", "Dad kneels beside the child" — these all draw Dad and break the policy.\n- The TEXT can say warm things about Dad; the IMAGE shows the child's world with Dad's signature objects.\nOther family members — text only, never illustrated.`;
    }
  }
...
async function planChapterBook(childDetails, theme, customDetails, opts = {}) {
  ...
  if (theme === 'mothers_day') {
    familyConstraint = additionalCoverCharacters
      ...
      : `\n\n⚠️ LOVE TO MOM OVERRIDE — TEXT-ONLY + SIGNATURE OBJECTS: Mom is a co-protagonist of this story IN THE WORDS, but she has NO reference image and is NOT on the cover. Mom must NEVER appear visually in any chapter illustration — not as a face, not as a body, not as a hand / arm / shoulder / back-of-head / silhouette / shadow / reflection. Mom is implied through her signature objects only (a tea mug, a folded cardigan, an empty rocking chair, reading glasses, slippers). Chapter illustrations show the CHILD experiencing the moment alone in the scene, with one of Mom's signature objects placed naturally. NEVER write image prompts that depict Mom's body or face in any form. Other family members (siblings, grandparents, dad) must NOT appear in illustrations — text only.`;
  } else if (theme === 'fathers_day') {
    familyConstraint = additionalCoverCharacters
      ...
      : `\n\n⚠️ FATHER'S DAY OVERRIDE — TEXT-ONLY + SIGNATURE OBJECTS: Dad is a co-protagonist of this story IN THE WORDS, but he has NO reference image and is NOT on the cover. Dad must NEVER appear visually in any chapter illustration — not as a face, not as a body, not as a hand / arm / shoulder / back-of-head / silhouette / shadow / reflection. Dad is implied through his signature objects only (a coffee mug, a folded jacket, work boots by the door, a baseball cap on a hook, a casual wristwatch on the counter). Chapter illustrations show the CHILD experiencing the moment alone in the scene, with one of Dad's signature objects placed naturally. NEVER write image prompts that depict Dad's body or face in any form. Other family members (siblings, grandparents, mom) must NOT appear in illustrations — text only.`;
...
async function planGraphicNovel(childDetails, theme, customDetails, opts = {}) {
  ...
  if (theme === 'mothers_day') {
    familyConstraint = additionalCoverCharacters
      ...
      : `\n\n⚠️ LOVE TO MOM OVERRIDE — TEXT-ONLY + SIGNATURE OBJECTS: Mom is a co-protagonist of this story IN THE WORDS, but she has NO reference image and is NOT on the cover. Mom must NEVER appear visually in any panel — not as a face, not as a body, not as a hand / arm / shoulder / back-of-head / silhouette / shadow / reflection. Mom is implied through her signature objects only (a tea mug, a folded cardigan, an empty rocking chair, reading glasses, slippers). Panels show the CHILD experiencing the moment alone in the scene, with one of Mom's signature objects placed naturally. NEVER write panel prompts that depict Mom's body or face in any form. Other family members (siblings, grandparents, dad) must NOT appear in illustrations — text only.`;
  } else if (theme === 'fathers_day') {
    familyConstraint = additionalCoverCharacters
      ...
      : `\n\n⚠️ FATHER'S DAY OVERRIDE — TEXT-ONLY + SIGNATURE OBJECTS: Dad is a co-protagonist of this story IN THE WORDS, but he has NO reference image and is NOT on the cover. Dad must NEVER appear visually in any panel — not as a face, not as a body, not as a hand / arm / shoulder / back-of-head / silhouette / shadow / reflection. Dad is implied through his signature objects only (a coffee mug, a folded jacket, work boots by the door, a baseball cap on a hook, a casual wristwatch on the counter). Panels show the CHILD experiencing the moment alone in the scene, with one of Dad's signature objects placed naturally. NEVER write panel prompts that depict Dad's body or face in any form. Other family members (siblings, grandparents, mom) must NOT appear in illustrations — text only.`;
  } else if (additionalCoverCharacters) {
    familyConstraint = `\n\n⚠️ COVER PHOTO OVERRIDE: The uploaded photo contains a secondary person (e.g. a parent/family member). This overrides the "no family in illustrations" rule for THIS book only. The following secondary character IS allowed in illustrations and must appear consistently:\n${additionalCoverCharacters}\nWrite their description into illustration prompts whenever they appear naturally in the scene. Do NOT invent other family members beyond what is listed above.`;
  } else {
    familyConstraint = `\n\nILLUSTRATION CONSTRAINT — NO FAMILY MEMBERS IN IMAGES:\nStory text MAY mention family members by name. However, family members must NEVER appear as visible characters in illustrations in any form — no face, no body, no hand, no shoulder, no silhouette, no shadow. We only have the child's photo. Design scenes so they center the child alone with the environment. Family presence is communicated through (a) the manuscript text and (b) signature OBJECTS placed naturally in the scene (a mug on the table, a coat on a hook, an empty chair) — never a body or limb.`;
  }
