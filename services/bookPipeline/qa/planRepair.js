...
function renderCorrectionNote(issues, tags, options = {}) {
  ...
  if (tagArr.includes('implied_parent_skin_mismatch')) {
    lines.push('The implied parent\'s visible skin (hands, fingers, wrist, forearm, sleeve cuff, neck/jaw if cropped torso, partial-face glimpse) does NOT match the HERO CHILD\'s skin tone on the BOOK COVER. They must read as the SAME family, SAME household, SAME ethnicity. Re-render every visible parent skin patch on this spread to match the cover child\'s skin EXACTLY — same lightness, same warmth, same undertone. If the cover child is fair / very pale, the parent\'s hand/arm is fair / very pale (NOT medium-tan, NOT olive). If the cover child is medium, the parent is medium. If the cover child is deep brown, the parent is deep brown. Use the cover child\'s skin patch as the literal color reference. A two-shade gap or any undertone shift is a hard fail — re-paint the parent\'s skin to be indistinguishable from the cover child\'s tone.');
  }
  if (tagArr.includes('full_body_parent_skin_mismatch')) {
    lines.push('A substantially-rendered adult (full face, partial face such as jaw/ear/cheek, full body, OR a large torso/arm presence) is sharing this spread with the hero, and that adult\'s skin tone clearly differs from the HERO CHILD on the BOOK COVER. They must read as the SAME family, SAME household. Re-render the adult\'s skin EXACTLY to the cover child\'s tone — same lightness, same warmth, same undertone, no two-shade gap, no medium-tan adult with a fair child, no warm-olive adult with a cool-fair child. If the parent is NOT on the cover per the off-cover policy AND the body presence is too large for the implied-only rule (substantial torso, full face, or full body), reduce the body presence to implied-only — hand / shoulder-back / cropped torso / shadow / off-frame object — AND match the cover child\'s skin tone on whatever skin patch remains visible. Use the cover child\'s skin patch as the literal color reference.');
  }
