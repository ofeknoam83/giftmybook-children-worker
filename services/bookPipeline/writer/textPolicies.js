  ...
  if (isPictureBook) {
    ...
    if (isInfant) {
      ...
      lines.push(
        ...
        '- RHYME SCHEME (AA-CW-18 — DEFAULT FREE-VERSE ON 3+4 FOR INFANT BOARD BOOKS): Lines 1+2 MUST rhyme — a real end-rhyme, no identity rhyme, no slant. Lines 3+4 DEFAULT to free-verse with a strong rhythmic parallel; only rhyme them if a real, natural rhyme appears in your FIRST attempt at the couplet. Lines 2 and 4 do NOT need to rhyme with each other. HARD PRINCIPLE: do NOT iterate to find a rhyme on lines 3+4. If your first line-3 ending does not have an obvious natural rhyme partner that fits the scene, lock the couplet as free-verse and move on. NEVER produce a rhyme that requires (a) meaning drift / nonsense ("dawning", "teal cheek", "cloth/both"), (b) an identity rhyme, (c) an invented prop, (d) an unrenderable action ("arm sighs", "leaf peeks"), or (e) a slant/r-controlled mismatch (cloth/both, sleeve/breeze, arm/warm). When you go free-verse on lines 3+4, declare it explicitly in `writerNotes` (e.g. "lines 3+4 unrhymed for natural cadence"). Real end-rhymes are welcome ONLY when they appear effortlessly on the first try.',
