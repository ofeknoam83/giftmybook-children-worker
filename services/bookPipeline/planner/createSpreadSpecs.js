...
${PLANNER_CREATIVE_BAR}
- **Variety + connection:** the book must travel across at least 4 visually distinct, **spectacular** moments (film-style: light, materials, scale or wonder) — **one connected journey**, not a slideshow of unrelated sets. Honor \`storyBible.visualJourneySpine\` and \`storyBible.recurringVisualMotifs\` when you pick \`location\` and \`focalAction\`. Prefer **revisiting 2–3 anchor places** under new angles or sequential beats on the **same outing** (same train/festival/route/pier complex) rather than inventing a unrelated typology every spread. Each spread after the first needs a **causal** reason to be there (following, carrying forward, consequence of last beat) — \`sceneBridge\` must name that handoff, not a vague mood. Avoid unmotivated "teleport" jumps and **questionnaire bingo** (a new random venue per spread just to use another parent detail).
...
function userPrompt(doc) {
  ...
  const parentDayLocationContract = parentDay
    ? [
      ...
      '- **Same thread:** outward spreads must still feel like **one shared outing** from `visualJourneySpine` — reuse or extend the **same** festival, waterfront, transit line, or civic hub across multiple beats (different plazas/platforms/rooms of the same complex) instead of a **new unrelated spectacle every spread** just to vary the map.',
