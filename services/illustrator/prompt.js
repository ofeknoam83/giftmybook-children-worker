...
function buildCharacterAnchorBlock(opts) {
  ...
  if (desc) {
    ...
    lines.push('- **REFERENCE IMAGES — what to match:** Reference 1 attached to this request is the BOOK COVER (canonical hero identity + canonical art style). Any additional references are previously APPROVED interior spreads from this same book — they show the SAME hero you must render here. Match the hero face, hair, skin tone, and outfit to those references EXACTLY. Treat the references as identity ground truth; the literal values in "Hero ground truth" above belt-and-suspenders the same description in text.');
    lines.push('- **OUTFIT LOCK (HARD — pre-render checklist):** before drawing this spread, look at the BOOK COVER reference image. Identify the exact garments the hero is wearing (top + graphic/print, bottom + pattern, footwear, headwear, hair accessory). Render the hero in THOSE EXACT garments — same graphic / print / lettering on the top, same pattern on the bottom, same colors, same silhouette, same hair accessory. Do NOT substitute a "similar plain version" (e.g. plain long-sleeved onesie) when the cover shows a printed graphic shirt. Do NOT drop the patterned pants for solid pants. Do NOT drop the headband / bow. Situational swaps (pajamas, swimwear, towel, snow coat) are allowed ONLY when this spread\'s SCENE block explicitly names that situation — every other scene wears the cover outfit, full stop.');
    lines.push('- **OUTFIT CONTINUITY (HARD):** every previously approved interior reference attached to this request shows the hero in the cover outfit. Match those interior images too — copy the visible top + bottom from the most recent approved spread reference exactly. If you find yourself reaching for a "simpler / cleaner" outfit because the current SCENE is calmer, STOP — wear the cover outfit anyway.');
  } else {
...
function buildTagDirectives(tags, opts) {
  ...
  if (set.has('outfit_mismatch')) {
    out.push(
      'Hero clothing drifted from the cover. Pre-render checklist: (1) look at the BOOK COVER image attached as Reference 1, AND the last accepted interior image(s) attached as later references; (2) read the hero outfit description in the system instruction and in this spread\'s CHARACTER ANCHOR block; (3) confirm the rendered hero in this image is wearing the SAME top with the SAME graphic/print/lettering, SAME bottom with SAME pattern, SAME footwear, SAME hair accessory as the cover and as your prior approved interiors. Do NOT substitute a plain solid-color onesie or a "simpler" version for a printed graphic top. Do NOT drop the patterned pants. Do NOT drop the headband/bow. Re-lighting, folds, and pose may change; the wardrobe must not. If this SCENE explicitly requires bath/pool/pajamas/snow layer per system rules, only then swap; otherwise the hero wears the cover outfit, character-for-character.',
    );
  }
  if (set.has('outfit_continuity_drift')) {
    out.push(
      'Hero outfit drifted vs the recent approved interior spread references attached to this request. The BOOK COVER (Reference 1) and the later interior references all show the hero in the same outfit (same graphic top, same patterned bottoms, same accessories). Re-render this spread with that SAME outfit, exactly. No "cleaner / calmer" variant for a quieter scene. If you find yourself drawing plain pajamas or a plain onesie because the SCENE feels gentle, STOP — wear the cover outfit unless the SCENE explicitly names a bath/pool/pajamas/snow situation.',
    );
  }
  if (set.has('hair_continuity_drift')) {
    out.push(
      'Hero hair drifted vs the cover and recent approved interior references attached to this request. Re-render with IDENTICAL hair color, length, texture, volume, and styling to the BOOK COVER reference and to the most recent approved interior reference — and IDENTICAL hair accessory (bow / headband / clip). Lighting can change brightness only; not color family, not length, not texture, not volume, not the accessory color or pattern.',
    );
  }
