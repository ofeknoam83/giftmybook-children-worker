...
function buildCharacterAnchorBlock(opts) {
  ...
  if (desc) {
    ...
    lines.push('- **NO INLINE IMAGE REFERENCES:** this is a stateless text-to-image call — there are no reference images attached. The hero appearance values listed above ARE the reference. Treat them as literal ground truth for face, hair, skin tone, and outfit; do not extrapolate, do not stylize away from them.');
    lines.push('- **OUTFIT LOCK (HARD — pre-render checklist):** the hero outfit description in "Hero ground truth" above lists the exact garments to render — top + graphic/print, bottom + pattern, footwear, headwear, hair accessory. Render the hero in THOSE EXACT garments — same graphic / print / lettering, same pattern, same colors, same silhouette, same hair accessory. Do NOT substitute a "similar plain version" (e.g. plain long-sleeved onesie) when the description names a printed graphic shirt. Do NOT drop the patterned pants for solid pants. Do NOT drop the headband / bow. Situational swaps (pajamas, swimwear, towel, snow coat) are allowed ONLY when this spread\'s SCENE block explicitly names that situation — every other scene wears the canonical outfit described above, full stop.');
    lines.push('- **OUTFIT CONTINUITY (HARD):** every spread of this book shows the hero in the same canonical outfit described above. If you find yourself reaching for a "simpler / cleaner" outfit because the current SCENE is calmer, STOP — wear the canonical outfit anyway.');
  } else {
...
function buildTagDirectives(tags, opts) {
  ...
  if (set.has('outfit_mismatch')) {
    out.push(
      'Hero clothing drifted from the canonical outfit described in the CHARACTER ANCHOR. Pre-render checklist: (1) re-read the hero outfit values in the CHARACTER ANCHOR block ABOVE — the outfit text lists the exact top (graphic/print/lettering), bottom (pattern), footwear, headwear, and hair accessory; (2) confirm the rendered hero in this image is wearing those EXACT garments. Do NOT substitute a plain solid-color onesie or a "simpler" version for a printed graphic top. Do NOT drop the patterned pants. Do NOT drop the headband/bow. Re-lighting, folds, and pose may change; the wardrobe must not. If this SCENE explicitly requires bath/pool/pajamas/snow layer per system rules, only then swap; otherwise the hero wears the canonical outfit, character-for-character. (Note: this is a stateless call — no inline reference images are attached. The CHARACTER ANCHOR text IS the reference.)',
    );
  }
  if (set.has('outfit_continuity_drift')) {
    out.push(
      'Hero outfit drifted vs the canonical outfit described in this spread\'s CHARACTER ANCHOR. The CHARACTER ANCHOR text lists the exact top (with graphic/print), bottom (with pattern), and accessories the hero wears in EVERY spread of this book. Re-render this spread with that exact outfit. No "cleaner / calmer" variant for a quieter scene. If you find yourself drawing plain pajamas or a plain onesie because the SCENE feels gentle, STOP — wear the canonical outfit unless the SCENE explicitly names a bath/pool/pajamas/snow situation.',
    );
  }
  if (set.has('hair_continuity_drift')) {
    out.push(
      'Hero hair drifted vs the canonical hero appearance described in the CHARACTER ANCHOR. Re-render with IDENTICAL hair color, length, texture, volume, and styling to the CHARACTER ANCHOR\'s "Hero ground truth" line — and IDENTICAL hair accessory (bow / headband / clip) to that description. Lighting can change brightness only; not color family, not length, not texture, not volume, not the accessory color or pattern.',
    );
  }
