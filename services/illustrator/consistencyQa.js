...
function buildConsistencyPrompt({
  ...
  "hairConsistentWithRecentInteriors": <true if the CANDIDATE hero's hair (color family, length, texture, style) is clearly consistent with the RECENT interior reference images and the COVER. Set false on any of these CLEAR drifts: COLOR FAMILY shift (dark-brown → light-brown, brown → blond, blond → red, brunette → ash); TEXTURE shift (curly mop → straight wispy, tight coils → loose waves, straight → curly); LENGTH/VOLUME shift (full thick mop → short wispy, long → short, dense → sparse). Lighting can change brightness or sheen; lighting CANNOT change color family, texture, or volume. Hair drift on a printed gift book is a hard fail — customers notice. ${isParentTheme ? 'For this parent-theme gift book, also false on subtle drift — when uncertain prefer false.' : 'When uncertain about a borderline case, prefer true; when the drift is named-and-obvious (e.g. dark-brown curly mop on cover → light-brown straight wispy hair on spread), prefer false.'}>,
