...
function buildConsistencyPrompt({ hasSecondaryOnCover, characterDescription, hasChildPhoto, qaAllowedHumansNote }) {
  ...
  const castNote = qaAllowedHumansNote
    ...
    : '';
  return `You are a quality reviewer comparing a book COVER (Image 1) to a generated interior SPREAD (Image 2). Both should be from the same 3D CGI Pixar **feature-film** children's book — a frame from a modern Pixar movie, photoreal subsurface skin scattering, rendered hair strands, physically based materials, volumetric ray-traced lighting, real optical depth-of-field. NOT a 2D illustration, NOT a painted children's book illustration, NOT a photograph.${photoNote}${desc}${castNote}
...
{
  "heroChildMatches": <true if the spread hero (Image 2) is clearly the same identity as the child on the cover (Image 1) — same apparent age band, same face and ethnicity, same skin tone, same **hair color and length class** and style, same eye color, within normal illustration variance (lighting/angle is not a pass for a clearly different age or much longer hair)>,
