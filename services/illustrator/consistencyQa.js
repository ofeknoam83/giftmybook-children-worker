...

{
  ...
  "heroOnFoldLine": <true if the HERO CHILD's body (head, torso, or most of the figure) overlaps the exact vertical centerline x=0.500 of the spread. This is the physical page fold — a hero standing on it is printed half on one page and half on the other (e.g. an arm on one page, the rest of the body on the next). Check the hero's bounding box against x=0.5: if the centerline passes through the hero's head or torso, this is true. A hand or foot merely reaching across is false; the fold passing through head/torso is true. This is INDEPENDENT of splitPanel — a perfectly coherent single scene still fails if the hero stands on the fold.>,
  "heroOnFoldLineNotes": "<short note if heroOnFoldLine is true (e.g. 'hero stands at center; the fold line passes through his torso'), else empty string>",
  "realWorldLogoOrFlag": <true if the spread depicts a REAL-WORLD brand logo or trademark (e.g. the NASA meatball, sports brands, car marques) or a NATIONAL FLAG on clothing, patches, vehicles, or props. Generic invented emblems (a plain star patch, a made-up rocket club badge with no letters) are false. This is a legal/brand-safety rule for a commercial product.>,
  "realWorldLogoOrFlagNotes": "<short note naming the logo/flag and where it appears (e.g. 'NASA logo on the vest, US flag on the sleeve'), else empty string>",
...
function evaluateConsistencyResult(parsed, recentInteriorCount = 0) {
  ...
  // Audit 2026-07-15: a shipped book had the hero bisected by the page fold
  // (an arm on one printed page, the body on the other). The layout engine
  // cuts the spread image 50/50 blindly, so the composition must keep the
  // hero off the centerline.
  if (parsed.heroOnFoldLine === true) {
    const notes = (parsed.heroOnFoldLineNotes || '').trim();
    issues.push(`Hero is positioned on the page-fold centerline and will be cut in half when the spread is split into two printed pages${notes ? `: ${notes}` : ''}`);
    tags.push('hero_in_gutter');
  }
  // Audit 2026-07-15 (book 2): real NASA logos + US flags shipped on the
  // hero's vest — trademark/brand-safety risk on a commercial product.
  if (parsed.realWorldLogoOrFlag === true) {
    const notes = (parsed.realWorldLogoOrFlagNotes || '').trim();
    issues.push(`Real-world brand logo or national flag in the artwork${notes ? `: ${notes}` : ''} — replace with a generic invented emblem`);
    tags.push('real_world_logo');
  }
