...

{
  ...
  "explicitStranger": <true ONLY if the spread shows a non-child human's FULL FACE or a FULL-BODY adult figure that is NOT visible on the cover. ${hasSecondaryOnCover ? 'Adults visible on the cover are allowed to appear fully — they are NOT strangers.' : 'When no adult is on the cover, a fully-rendered adult (face visible, or full body figure) is a failure.'} Two full adults (e.g. a man and a woman) pushing a stroller or walking beside the child when no adults are on the cover = explicitStranger true. Hands, arms, back-of-head, a shoulder, or a cropped torso with the face out of frame are NOT explicit — do not flag those as strangers. Implied/partial adult presence is expected when the story only needs a caregiver's presence, not a portrait. Distant, soft silhouettes without facial detail are usually NOT strangers unless they read as two full parents beside the child.>,
