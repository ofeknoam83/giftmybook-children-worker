...
function buildSystemInstruction(opts) {
  ...
  sections.push(
isSquare
? `You are the illustrator for a premium ${TOTAL_SPREADS}-spread children's picture book. This is a STATELESS text-to-image generation call — NO reference images are attached to the request. You will receive ${TOTAL_SPREADS} per-spread prompts, each one self-contained. For each prompt, generate ONE 1:1 square full-bleed illustration that obeys every rule in this system instruction and every literal value (hero appearance, outfit, hair, skin tone) in the per-spread CHARACTER ANCHOR. The manuscript caption is rendered as PDF text on the FACING PAGE — your image must contain NO text. Every spread must read as the same canonical 3D CGI Pixar feature-film render — same hero, same outfit, same style — based on the literal text descriptions you receive each call.`
