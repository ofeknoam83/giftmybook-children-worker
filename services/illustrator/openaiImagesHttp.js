 ...
 *
 * **gpt-image-2 + reference images:** Use `POST /v1/images/edits` with
 * multipart `model`, `prompt`, `size`, `quality`, and `image[]`. The
 * `/v1/images/generations` endpoint only accepts JSON in production, so it
 * cannot carry reference images for gpt-image-2 even though older docs hinted
 * otherwise. `postImagesGenerations` (multipart) is kept as a fallback only.
 *
...
/**
 * POST /v1/images/edits — multipart upload of reference images plus prompt.
 * For `gpt-image-2` this is the production endpoint that accepts `image[]`
 * + `quality`; older API docs that hinted at multipart on `/generations`
 * are wrong (see file header).
 * @param {object} p
 * @param {string} p.apiKey
 * @param {string} p.model
 * @param {string} p.prompt
 * @param {string} [p.size]
 * @param {string} [p.quality] - `low` | `medium` | `high` | `auto`
 * @param {Array<{ buffer: Buffer, filename: string, mimeType?: string }>} p.imageFiles
 * @param {string} [p.url]
 * @param {number} [p.timeoutMs]
 * @param {AbortSignal} [p.signal]
