...
async function checkLineSheet(buffer, o = {}) {
  ...
  const fields = ['"readable_text": true|false,   // any readable text, letters, labels, numbers, or watermarks — a BLANK patch, badge or label shape on a garment (its letters left out) is a garment, not text', '"shading_present": true|false, // grey tones, hatching, washes, gradients', '"solid_fills": true|false,     // solid black areas beyond tiny accents', '"open_shapes": true|false,     // outlines with visible gaps'];
  ...
  if (o.kind === 'hero') {
    lines.push('Image 2 is the COLOUR character model sheet the candidate must reproduce: the SAME single child, THREE full-body figures (front, three-quarter, back) in the same order, the same hair shape and length, the same face and proportions, and the same complete outfit garment by garment (cut, length, pattern — colours do not exist in line art), plus two small head insets. Flat white background, no scene. Lettering that image 2 shows on a garment (a logo, patch, badge, name or number) is drawn in line art as that garment\'s blank outline shape: the shape stays, the letters go.');
    if (o.outfitSpecText) lines.push(`Outfit spec (colours removed): "${qaData(o.outfitSpecText, 500)}".`);
    fields.push('"figure_count": <integer>,     // FULL-BODY figures (head insets do not count)', '"one_child": true|false,       // every figure is the SAME single child', '"same_child": true|false,      // the child matches image 2 (face, hair, proportions)', '"outfit_match": true|false,    // the outfit matches image 2 garment by garment in every view (a logo, patch or badge that image 2 letters and the candidate draws as a blank outline shape is the SAME garment, not a difference)', '"likeness": <number 0.0-1.0>   // how well the figures match image 2\'s child');
