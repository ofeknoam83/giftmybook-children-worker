...
async function buildBookBible(p) {
  ...
  return { manifest, hash: manifest.bibleHash, sheet, outfit, props, companion, worldPlate, emotion, storyObjects, advisories };
}
...
function buildReferencePack(bible, ctx) {
  ...
  if (ctx.typographyAnchor && ctx.typographyAnchor.base64) {
    ...
    refs.typographyRef = push({ kind: a.kind === 'template' ? 'typography-template' : 'typography', label: a.kind === 'template' ? `FULL-SPREAD LETTERING TEMPLATE — EDIT BASE: this is the ENTIRE 16:9 canvas at full height, not a crop. It already contains THIS spread’s exact manuscript and line breaks in the ${side.toUpperCase()} column, in Playfair Display Regular at ${a.capHeightPercent}% cap height with ${a.inkHex} fill. Preserve these small glyphs at their exact canvas-relative size, position, line spacing, weight and colour. Paint the requested scene throughout ALL transparent space, including between and behind the letters. The transparency is missing artwork to complete, never a panel or empty area to keep. Do not zoom, enlarge, reflow or restyle the text. Use the other references for the child and world; return the completed full-spread illustration with this lettering embedded in it.` : a.kind === 'guide' ? `TYPOGRAPHY SIZE AND INK GUIDE: this transparent image is the LEFT 45% of a full 16:9 illustration, at FULL page height. Its small glyphs are the exact target scale: match their cap height (${a.capHeightPercent}% of the full height), regular serif face, and ${a.inkHex} fill on EVERY spread, including spread one. Do not enlarge the letters to fill the reference or text column. This is a lettering-only proof, NOT a finished scene. Ignore the blank/transparent backing completely; paint the words directly into your own continuous scenery, without a box, panel, fog or background patch. Never copy the sample words. Use only this spread’s supplied manuscript, its own line breaks and assigned side.` : `TYPOGRAPHY REFERENCE (page ${a.spread} of THIS book, already painted — the ${side} half of that page at full height): match its story text's typeface, weight, and SIZE relative to the page height EXACTLY (each row of your text as tall as one of its rows), and above all its INK COLOUR — the same dark cocoa-brown fill with the same thin pale hairline, never inverted to light text and never retinted to suit your scene — painted straight over the sharp, fully detailed scene the same way (no blur, fog, or darkening behind the letters). TYPE ONLY — never copy its words, its scenery, its composition, or anything else from it.`, base64: a.base64, mimeType: a.mimeType || 'image/png' });
  }
  return { pack, refs };
}
