...
function _buildCharacterEstablishmentPrompt(session) {
  ...
  parts.push(`- EDGE PADDING: Leave at least 8% from left and right, at least ${tr.topPaddingPercent ?? tr.cornerVerticalPaddingPercent}% from the TOP and at least ${tr.bottomPaddingPercent ?? tr.cornerVerticalPaddingPercent}% from the BOTTOM, so text is not cut when printed. Text must have visible breathing room from every edge.`);
