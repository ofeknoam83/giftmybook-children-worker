...
function buildCharacterPrompt(sceneDescription, artStyle, childName, pageText, characterOutfit, characterDescription, recurringElement, keyObjects, opts = {}) {
  ...
  if (opts.embedText && pageText && pageText.trim()) {
    ...
    parts.push('- TEXT PLACEMENT (CRITICAL): Text can be anywhere EXCEPT it must NEVER cross or touch the horizontal center line (the halfway point between top and bottom). The ENTIRE text block must sit in the top half OR bottom half of the image.');
    ...
    parts.push('\nREMINDER: Text must NEVER cross the horizontal center line. Keep text in the top half OR bottom half only. Leave at least 8% padding from ALL edges.');
  } else {
  ...
  if (opts.embedText && pageText && pageText.trim()) {
    parts.push(`10. TEXT RENDERED: story text is included exactly as provided, in the top half or bottom half only, never crossing the horizontal center, with at least 8% padding from all edges. \u2713`);
  } else {
