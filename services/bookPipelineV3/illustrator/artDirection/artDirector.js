...
` : ''}- FAMILY IN ART: family members (parents, grandparents, siblings) are excluded from the visual cast upstream unless this is a parent-day book (only then does the celebrated Mom or Dad appear). NEVER reintroduce a family member into a moment, poseHint, continuityNotes, or cast lock the contracts don't list — a caregiver named in the story TEXT is implied in the art only as an effect (a warm hand at the frame edge, a light from a doorway, a tucked blanket), never as a person with a face.
