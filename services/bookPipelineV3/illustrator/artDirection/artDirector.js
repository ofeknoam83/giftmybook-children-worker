...
- CAST LOCKS: every NAMED individual supporting character who appears on 2+ spreads (Mom, Dad, Grandma, a named friend) gets a continuityLocks.cast entry with a LOCKED design — hair color+style, skin tone, one specific outfit (2026-07-28 audit, book 16758e3c: Mom and Dad swam in swimwear on one spread and sat fully dressed as different-looking people two pages later; the hero was locked to the cover but the family had NO ground truth). Never lock the child hero (the model sheet owns them) and never lock declared GROUPS. Keep designs simple to draw consistently, letter-free, logo-free.
- GEAR STATE: if the hero wears removable gear (a helmet, goggles, a backpack), define a continuityLocks.gear rule tied to settings (2026-07-19 audit: a space helmet appeared and vanished at random across outdoor Mars scenes — vacuum logic that parents notice) AND state the gear state explicitly in EVERY spread's continuityNotes (e.g. "helmet dome ON — outdoors").${embedded ? `
