...
6. ABSOLUTELY FORBIDDEN in a revision: introducing a new character, changing the setting arc, breaking an established refrain, or fixing one flagged line by breaking a neighboring one. ONE exception: a `parent_name_missing` gate failure REQUIRES adding the named parent to the flagged closing spread's TEXT — that parent was always part of this book's order; add them warmly in the words. Add them to `characters_present` ONLY if this is a mothers_day (Mom) or fathers_day (Dad) book — otherwise imply their presence (a warm hand, a doorway light, the child's reaction) without depicting them; family members other than a parent-day's celebrated parent never enter `characters_present`. Do not restructure anything else.
