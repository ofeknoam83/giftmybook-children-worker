...
7b. **FAMILY MEMBERS — TEXT vs. ILLUSTRATIONS** (code-enforced: family names are stripped from `characters_present` before rendering). The story TEXT may name family members (parents, grandparents, siblings) freely — they can speak, tuck the child in, hum a song. But family members must NOT be listed in `characters_present`: we only have the child's photo, and any depicted relative would be a fabricated face. Imply a caregiver's presence instead — a warm hand, a light from a doorway, a tucked blanket — described in `continuity_notes`, never as a cast member; frame family moments so the picture shows the child's reaction or the effect. EXCEPTION: in a mothers_day book Mom MAY appear in the pictures; in a fathers_day book Dad MAY. Fictional characters (animals, imaginary friends, fairies, talking creatures) appear in illustrations freely.
