...
2. **emotional_truth** — Is the arc earned? Does the climax pay off what the setup planted, or does the emotion arrive by assertion? **Mood is not story: a manuscript that reads as a mood collage — atmosphere and imagery with no causal chain of events a listener could retell (this happened, so then this happened: problem → attempt → turn → resolution) — scores AT MOST 3 here, regardless of how pretty the lines are.**
3. **page_turn_pull** — Does each spread end with a reason to turn? Score the weakest stretch, not the average. **The mood-collage cap applies here too: disconnected images generate no pull; if events merely occur near each other instead of causing each other, score AT MOST 3.**
4. **concrete_specificity** — Things you can see, touch, hear. Abstractions ("so much fun", "filled with joy") cost points.
5. **personalization_depth** — Would this story survive with a different child's details swapped in? It must NOT. The child's real details (from the brief) must be load-bearing in the plot. **If the brief lists `interests`, the story's world or premise must visibly engage the strongest of them — a story that ignores a stated interest (a space-loving child handed a generic jungle adventure) scores AT MOST 3 on this dimension, regardless of craft.** **The same cap applies to the ordered `themes`: if `themes.storyTheme` is set, the story must visibly live in that world; if `themes.occasion` is set, the book must serve it (a bedtime order that ends on a loud cliffhanger, or a birthday order where the birthday is set dressing, scores AT MOST 3 here).** **The same cap applies to `brief.storyRoles` — the machine-built casting sheet of the parent's questionnaire inputs: a provided role that the story does not use as a PLOT MECHANIC (the hobby-tool never makes progress, the funny trait does not cause the resolution, the food is scenery instead of a world object, a named parent is absent from the ending), or a funny trait rendered as dialogue/an exclamation instead of demonstrated behaviour ("calls everything mama" must be shown, not shouted), scores AT MOST 3 here. An input mentioned once and never used again is decoration, not personalization.**
6. **age_fit** — Vocabulary, sentence length, emotional register, and attention shape for this band. Judge conflict intensity against the profile's `antagonistPolicy`: the younger bands allow NO antagonist character (obstacles from nature/time/uncertainty only); the early-reader band allows a mild, never-scary antagonist that must end outwitted or befriended — an antagonist where the policy forbids one, or a frightening/violently-defeated one anywhere, caps this dimension at 3.
