...
- **Model routing** (`llm/modelRouter.js`): BRIEF/CONCEPT/WRITER → `claude-opus-4-8`; EDITOR → `gpt-5.4`; judges = claude + gpt-5.4 + gemini-2.5-pro (three families, blind, median ≥ 4 on all 7 dimensions to pass). Override per deploy via `BOOK_PIPELINE_V3_<ROLE>_FAMILY` / `_TIER`. `ANTHROPIC_API_KEY` is required at run time — `assertV3Config()` fails the book before any LLM spend if a routed family has no key. No silent family fallback.
