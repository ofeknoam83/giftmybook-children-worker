...
function buildStoryPlannerSystem(vars, additionalCoverCharacters = null, theme = null) {
  ...
  if (theme === 'mothers_day') {
    if (additionalCoverCharacters) {
      ...
      const combinedOverride = `LOVE TO MOM — MOM AS VISIBLE CHARACTER + SECONDARY CHARACTERS:
...
CRITICAL: Their appearance must be CONSISTENT across all illustrations. Only Mom and the secondary character(s) listed above are allowed in illustrations — do NOT invent any other family members.`;
      ...
      // Love to mom theme WITHOUT a cover parent: Mom is in the STORY TEXT but shown via implied presence in illustrations
      const motherImpliedOverride = `LOVE TO MOM — MOM IN STORY, IMPLIED PRESENCE IN ILLUSTRATIONS:
...
function getThemeContext(theme, additionalCoverCharacters = null) {
  switch (theme) {
    case 'birthday':
    ...
    case 'mothers_day':
      if (additionalCoverCharacters) {
        return `\n\nTHEME CONTEXT — LOVE TO MOM:\nThis is a love letter from a child to their mother. Mom is a NAMED, VISIBLE co-protagonist — she appears alongside the child in at least 6 of 13 spreads. The story is told from the child's perspective: gratitude, love, and a desire to show Mom how special she is. The arc moves from everyday shared moments to a heartfelt gesture of love. Mom must be described consistently in every illustration prompt where she appears. The ending is warm, tender, and emotionally resonant — it should make a mother cry happy tears.\n\nMOM IN ILLUSTRATIONS (CRITICAL): Mom is allowed — and required — in illustration prompts for this theme. Describe her presence explicitly when she appears (position, gesture, expression). Use the specific appearance description from the cover consistently. Do NOT apply the "never depict family members" rule to Mom in this theme.`;
      }
      return `\n\nTHEME CONTEXT — LOVE TO MOM:\nThis is a love letter from a child to their mother. Mom is a co-protagonist — she appears alongside the child in at least 6 of 13 spreads. The story is told from the child's perspective: gratitude, love, and a desire to show Mom how special she is. The arc moves from everyday shared moments to a heartfelt gesture of love. The ending is warm, tender, and emotionally resonant — it should make a mother cry happy tears.\n\nMOM IN ILLUSTRATIONS — IMPLIED PRESENCE ONLY (CRITICAL):\nWe have NO reference image for Mom, so her face must NEVER be shown explicitly in illustrations. She is FEMALE — always a woman, never a man.\nMom MUST appear physically present in illustration prompts for at least 6 of 13 spreads — she is REAL, not invisible.\nBut ALWAYS describe her with HIDDEN FACE: show her from behind, from the side with face turned away, as hands reaching in from the frame edge, kneeling with face cropped out, back view hugging the child, silhouette, or partially out of frame.\nNEVER write "a warm-smiled woman" or describe Mom's facial features in spread_image_prompt fields.\nDO describe: her hands, her arms around the child, her back, her hair from behind, her silhouette, her posture.\nThe child's face is always fully visible. Mom's warmth comes through her ACTIONS and BODY LANGUAGE, not her face.\nDo NOT apply the "never depict family members" rule to Mom — she IS present, just with her face hidden.`;
    case 'fathers_day':
      if (additionalCoverCharacters) {
        return `\n\nTHEME CONTEXT — FATHER'S DAY:\nThis is a love letter from a child to their father. Dad is a NAMED, VISIBLE co-protagonist — he appears alongside the child in at least 6 of 13 spreads. The story is told from the child's perspective: gratitude, love, and admiration for Dad. The arc moves from everyday shared moments (playing, building, exploring) to a heartfelt gesture of love. Dad must be described consistently in every illustration prompt where he appears. The ending is warm, tender, and emotionally resonant — it should make a father's eyes water.\n\nDAD IN ILLUSTRATIONS (CRITICAL): Dad is allowed — and required — in illustration prompts for this theme. Describe his presence explicitly when he appears (position, gesture, expression). Use the specific appearance description from the cover consistently. Do NOT apply the "never depict family members" rule to Dad in this theme.`;
      }
      return `\n\nTHEME CONTEXT — FATHER'S DAY:\nThis is a love letter from a child to their father. Dad is a co-protagonist — he appears alongside the child in at least 6 of 13 spreads. The story is told from the child's perspective: gratitude, love, and admiration for Dad. The arc moves from everyday shared moments (playing, building, exploring) to a heartfelt gesture of love. The ending is warm, tender, and emotionally resonant — it should make a father's eyes water.\n\nDAD IN ILLUSTRATIONS — IMPLIED PRESENCE ONLY (CRITICAL):\nWe have NO reference image for Dad, so his face must NEVER be shown explicitly in illustrations. He is MALE — always a man, never a woman.\nDad MUST appear physically present in illustration prompts for at least 6 of 13 spreads — he is REAL, not invisible.\nBut ALWAYS describe him with HIDDEN FACE: show him from behind, from the side with face turned away, as hands reaching in from the frame edge, kneeling with face cropped out, back view hugging the child, silhouette, or partially out of frame.\nNEVER write "a warm-smiled man" or describe Dad's facial features in spread_image_prompt fields.\nDO describe: his hands, his arms around the child, his back, his hair from behind, his silhouette, his posture.\nThe child's face is always fully visible. Dad's warmth comes through his ACTIONS and BODY LANGUAGE, not his face.\nDo NOT apply the "never depict family members" rule to Dad — he IS present, just with his face hidden.`;
    default:
      return '';
  }
}

function buildStoryWriterSystem(vars, theme, additionalCoverCharacters = null) {
  const themeContext = getThemeContext(theme, additionalCoverCharacters);
 ...
 * @returns {string}
 */
function STORY_WRITER_USER(childDetails, theme, customDetails, v2Vars = {}, additionalCoverCharacters = null) {
  const name = sanitizeForPrompt(childDetails.childName || childDetails.name || '', 50);
  const age = childDetails.childAge || childDetails.age || 5;
  const interests = (childDetails.childInterests || childDetails.interests || []).map(i => sanitizeForPrompt(i, 50)).join(', ') || 'general';
  const details = customDetails ? sanitizeForPrompt(customDetails, 500) : '';
...
When weaving the child's interests into the story, include at least one clearly recognizable mention (name, visual, or unmistakable motif) alongside any playful puns or allusions, so personalization is legible to both parents and downstream illustration systems.`;
  ...
  const themeJourneyRules = {
    ...
    mothers_day: additionalCoverCharacters
      ? `\n\nLOVE TO MOM THEME — LOVE LETTER ARC (CRITICAL):\nThis is a Love to mom book — a love letter from the child to Mom.\n- Mom is a NAMED, VISIBLE co-protagonist. Use the name from customDetails (calls_mom / mom_name). If not provided, use "Mommy".\n- Mom MUST appear in illustration prompts for at least 6 of 13 spreads — describe her position, gesture, and expression each time.\n- The story is told from the child's perspective of love and gratitude.\n- Arc: shared moments → imaginative play (child transforms the ordinary into magic, Mom plays along) → child notices Mom / Mom notices child → child tries to take care of Mom (reversal) → child gives Mom an imperfect, precious gift → joyful celebration → warm, bright closing.\n- Include the meaningful_moment from customDetails as a specific scene.\n- CELEBRATION ONLY: NO tantrums, NO crying, NO frustration, NO anger, NO conflict, NO "even when" moments. Every spread must be warm, joyful, and positive.\n- NO BEDTIME ENDING: The story must NOT end with sleeping, goodnight, tucking in, dreams, nightlights, or the house going quiet. End in DAYLIGHT with warmth, togetherness, and joy.\n- CREATIVITY: At least 2 spreads must use the child's imagination — transforming something ordinary into something magical. Include one reversal where the child tries to take care of Mom. Avoid flat documentary narration ("they did this, then they did that").\n- The ending is warm, bright, celebratory — Mom and child together in daylight, full of love and energy. NOT quiet, NOT sleepy.\n- IGNORE the "never depict family members" rule for Mom. Mom is a visible character in illustrations for this theme.\n- The repeated phrase should deepen in meaning each time it appears — same words, shifting context.\n- This book should make a mother cry happy tears.`
      : `\n\nLOVE TO MOM THEME — LOVE LETTER ARC (CRITICAL):\nThis is a Love to mom book — a love letter from the child to Mom.\n- Mom is a co-protagonist. Use the name from customDetails (calls_mom / mom_name). If not provided, use "Mommy".\n- Mom MUST appear in illustration prompts for at least 6 of 13 spreads.\n- CRITICAL — IMPLIED PRESENCE ONLY: We have NO reference image for Mom. She is FEMALE (a woman — never a man). Her face must NEVER be described in spread_image_prompt fields. In every illustration prompt where Mom appears, describe her with HIDDEN FACE: back view, hands reaching in, side view with face turned away, silhouette, kneeling with face cropped out, or partially out of frame. NEVER write her expression or facial features. Describe: her hands, arms, back, hair from behind, posture.\n- The story is told from the child's perspective of love and gratitude.\n- Arc: shared moments → imaginative play (child transforms the ordinary into magic, Mom plays along) → child notices Mom / Mom notices child → child tries to take care of Mom (reversal) → child gives Mom an imperfect, precious gift → joyful celebration → warm, bright closing.\n- Include the meaningful_moment from customDetails as a specific scene.\n- CELEBRATION ONLY: NO tantrums, NO crying, NO frustration, NO anger, NO conflict, NO "even when" moments. Every spread must be warm, joyful, and positive.\n- NO BEDTIME ENDING: End in DAYLIGHT with warmth, togetherness, and joy.\n- CREATIVITY: At least 2 spreads must use the child's imagination. Include one reversal where the child tries to take care of Mom. Avoid flat documentary narration.\n- The ending is warm, bright, celebratory — Mom and child together in daylight. NOT quiet, NOT sleepy.\n- Mom IS allowed in illustrations (override the "never depict family members" rule) — but always with her face hidden.\n- The repeated phrase should deepen in meaning each time it appears — same words, shifting context.\n- This book should make a mother cry happy tears.`,
    fathers_day: additionalCoverCharacters
      ...
      : `\n\nFATHER'S DAY THEME — LOVE LETTER ARC (CRITICAL):\nThis is a FATHER'S DAY book — a love letter from the child to Dad.\n- Dad is a co-protagonist. Use the name from customDetails (calls_dad / dad_name). If not provided, use "Daddy".\n- Dad MUST appear in illustration prompts for at least 6 of 13 spreads.\n- CRITICAL — IMPLIED PRESENCE ONLY: We have NO reference image for Dad. He is MALE (a man — never a woman). His face must NEVER be described in spread_image_prompt fields. In every illustration prompt where Dad appears, describe him with HIDDEN FACE: back view, hands reaching in, side view with face turned away, silhouette, kneeling with face cropped out, or partially out of frame. NEVER write his expression or facial features. Describe: his hands, arms, back, hair from behind, posture.\n- The story is told from the child's perspective of love and admiration.\n- Arc: everyday shared moments (playing, building, exploring) → realization of Dad's love → child's gesture of love back → heartfelt embrace.\n- Include the meaningful_moment from customDetails as a specific scene.\n- The ending is warm, tender, deeply emotional — Dad and child together. NOT a quest ending, NOT a bedtime ending.\n- Dad IS allowed in illustrations (override the "never depict family members" rule) — but always with his face hidden.\n- The repeated phrase should feel tender and personal in its final appearance.\n- This book should make a father's eyes water.`,
  };
