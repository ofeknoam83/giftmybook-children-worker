...
Theme: ${theme || 'bedtime'}`;
  ...
  if (theme === 'birthday') {
    ...
    prompt += `\n\n⚠️ BIRTHDAY ENDING RULE — SPREAD 13 MUST BE THE CAKE/CANDLES MOMENT:\nThis rule applies to birthday theme only. The entire story arc must build toward spread 13 as the natural, earned payoff.\n\nWrite spread 13 text in your own voice — warm, joyful, matching the child's voice and the rhythm of the story that came before. Do NOT use a template sentence.\n\nThe ILLUSTRATION for spread 13 is fixed and non-negotiable: ${name} leaning toward a birthday cake, cheeks puffed, about to blow out the candles. Warm golden candlelight, confetti, joy.\nCANDLE COUNT: The cake must have EITHER:\n  • Exactly ${candleCount} individual lit candles (one for each year), OR\n  • A single large numeral candle in the shape of the number ${candleCount}\nDo NOT draw ${candleCount + 1} candles or any other number. This is the child's ${candleCount}${candleCount === 1 ? 'st' : candleCount === 2 ? 'nd' : candleCount === 3 ? 'rd' : 'th'} birthday.\nShape the preceding spreads so this ending feels inevitable, not sudden.`;
  }
