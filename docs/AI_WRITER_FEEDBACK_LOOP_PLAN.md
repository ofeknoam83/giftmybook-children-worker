...
1. **Playground** — setup form covering every customer field that reaches the writer (theme, occasion, name/age/pronouns, all profile details), **editable mid-session** (saving re-selects the slate; past rounds keep their snapshots and cross-change comparisons are flagged "inputs changed") with **one-click auto-fill** from realistic sample children; candidate pinning (a workbench comparison tool — customers never pick a plot) and a version selector → story cards: spreads, **judge scorecard** (7 trait chips, click-to-comment), validation status, comment rail with scope selector; iteration timeline with side-by-side diff + trait deltas; "Apply comments" diff-review modal (scope confirmation + conflict resolution + route review).
...
The anatomy also travels to the story: a shared **StoryAnatomy** panel ("How this story was made") renders inline on every finished story — workbench story cards and the candidate slate on AdminChildrenBookDetails — reading the story's own pinned versions echo and resolving the tuning tag to the exact directives that applied to that request (`POST /story-anatomy`, with a drift warning when a draft changed post-generation).
